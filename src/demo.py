...
TEST_DIR = os.path.join(BASE_DIR, "data", "test_real_xray")
...
def run_ultimate_demo():
    ...
    if os.path.exists(MODEL_CYCLE_PATH):
        ...
        print("⚠️ CycleGAN model not found! Using U-Net only.")
        USE_CYCLEGAN = False
    ...
    # Step 2: U-Net Segmentation (both with and without CycleGAN)
    with torch.no_grad():
    ...
    print(f"   With CycleGAN: {pred_angle_cycle}° (Error: {error_cycle:.1f}°)" if error_cycle else f"   With CycleGAN: {pred_angle_cycle}")
    print(f"   Without CycleGAN: {pred_angle_direct}° (Error: {error_direct:.1f}°)" if error_direct else f"   Without CycleGAN: {pred_angle_direct}")
